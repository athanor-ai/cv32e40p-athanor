...
CASES = {
    "positive_fpu0_zfinx0": {"expect": "success", "path": ROOT / "full_decoder_equiv/positive_fpu0_zfinx0"},
    "positive_fpu1_zfinx0": {"expect": "success", "path": ROOT / "full_decoder_equiv/positive_fpu1_zfinx0"},
    "positive_fpu1_zfinx1": {"expect": "success", "path": ROOT / "full_decoder_equiv/positive_fpu1_zfinx1"},
    "negative_missing_bit12": {"expect": "fail", "path": ROOT / "full_decoder_equiv/negative_missing_bit12"},
    "negative_missing_bit2": {"expect": "fail", "path": ROOT / "full_decoder_equiv/negative_missing_bit2"},
}


def run_case(path: Path) -> dict:
    proc = subprocess.run([str(YOSYS), "-s", "prove.ys"], cwd=path, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=120)
    text = proc.stdout + proc.stderr
    return {"rc": proc.returncode, "success": "no model found: SUCCESS" in text, "fail": "model found: FAIL" in text}


def main() -> int:
    expected = json.loads((ROOT / "full_decoder_equiv/summary.json").read_text())
    failures = []
    rows = {}
