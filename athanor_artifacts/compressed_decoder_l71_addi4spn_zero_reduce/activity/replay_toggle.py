...
OUT = Path(os.environ.get("OUT_DIR", "/tmp/cv32-compressed-l71-activity"))
...
module tb;
  ...
  function [15:0] c0_addi4spn;
    input [7:0] imm125;
    input [2:0] rdprime;
    begin
      c0_addi4spn = {3'b000, imm125, rdprime, 2'b00};
    end
  endfunction
  ...
  function [31:0] fetch_word;
    ...
    begin
      case (c % 12)
        0: lo = c0_addi4spn(8'h00, 3'h0); // exact changed zero branch
        1: lo = c0_addi4spn(8'h80, 3'h1); // bit 12 nonzero
        2: lo = c0_addi4spn(8'h01, 3'h2); // bit 5 nonzero
        3: lo = c0_addi4spn(8'h55, 3'h3); // mixed nonzero immediate
        4: lo = 16'h0001; // c.nop
        ...
        default: lo = 16'h0000;
      endcase
      hi = c0_addi4spn(((c * 17) ^ 8'h5a) & 8'hff, c[2:0]);
      fetch_word = {hi, lo};
    end
  endfunction
...
def main() -> int:
    ...
    result = {
        ...
        "candidate": "compressed_l71_addi4spn_zero_reduce",
