...
CASES = {
    ...
    "negative_wrong_demorgan": {"path": ROOT / "predicate_equiv/negative_wrong_demorgan", "expect": "fail"},
    "negative_nextcnt_missing_down": {"path": ROOT / "predicate_equiv/negative_nextcnt_missing_down", "expect": "fail"},
}
...
def main() -> int:
    ...
    receipt = {"candidate": "prefetch_hwlp_plus_next_cnt_arith", "rows": rows, "expected_log_sha_fields": {k: v["log_sha"] for k, v in expected["rows"].items()}}
